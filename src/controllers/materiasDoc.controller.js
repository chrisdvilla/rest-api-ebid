...
export const getMaterias = async (req, res) => {
   ...
   try {
    const [rows] = await pool.query("SELECT * FROM paralelo JOIN curso ON paralelo.id_materia = curso.id JOIN horario ON horario.id_set = paralelo.id WHERE paralelo.docente = ? AND ", [
        req.params.docente,
      ]);
    res.json(rows);
  } catch (error) {
